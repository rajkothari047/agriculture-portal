...
$farm_size = isset($row4['farm_size']) ? $row4['farm_size'] : 'Not Specified';
$primary_crop = isset($row4['primary_crop']) ? $row4['primary_crop'] : 'Not Specified';
$experience_years = isset($row4['experience_years']) ? $row4['experience_years'] : '0';
$aadhar_number = isset($row4['aadhar_number']) ? $row4['aadhar_number'] : 'Not Added';
$pan_number = isset($row4['pan_number']) ? $row4['pan_number'] : 'Not Added';
$bank_account = isset($row4['bank_account']) ? $row4['bank_account'] : 'Not Added';
$ifsc_code = isset($row4['ifsc_code']) ? $row4['ifsc_code'] : 'Not Added';
$organic_certified = isset($row4['organic_certified']) ? $row4['organic_certified'] : 'No';
$fpo_member = isset($row4['fpo_member']) ? $row4['fpo_member'] : 'No';
$pm_kisan_status = isset($row4['pm_kisan_status']) ? $row4['pm_kisan_status'] : 'Eligible';
...
if(isset($_POST['farmerupdate'])) {
    ...
    $farm_size_val = mysqli_real_escape_string($conn, $_POST['farm_size']);
    $primary_crop_val = mysqli_real_escape_string($conn, $_POST['primary_crop']);
    $experience_years_val = mysqli_real_escape_string($conn, $_POST['experience_years']);
    $aadhar_val = mysqli_real_escape_string($conn, $_POST['aadhar_number']);
    $pan_val = mysqli_real_escape_string($conn, $_POST['pan_number']);
    $bank_val = mysqli_real_escape_string($conn, $_POST['bank_account']);
    $ifsc_val = mysqli_real_escape_string($conn, $_POST['ifsc_code']);
    $organic_val = isset($_POST['organic_certified']) ? 'Yes' : 'No';
    $fpo_val = isset($_POST['fpo_member']) ? 'Yes' : 'No';
    ...
    $updatequery1 = "UPDATE farmerlogin SET 
        ...
        farm_size='$farm_size_val',
        primary_crop='$primary_crop_val',
...
if(isset($_POST['upload_profile_pic'])) {
    $target_dir = "assets/uploads/FProfile/";
    ...
    if(isset($_FILES["profile_pic"]) && $_FILES["profile_pic"]["error"] == 0) {
        ...
        if(in_array($imageFileType, $allowed_types)) {
            if(move_uploaded_file($_FILES["profile_pic"]["tmp_name"], $target_file)) {
                ...
                $update_pic = "UPDATE farmerlogin SET profile_pic='$db_file_path' WHERE farmer_id='$para1'";
                if(mysqli_query($conn, $update_pic)) {
                    echo "<script>alert('Profile picture updated successfully!'); window.location='fprofile.php';</script>";
                    exit();
                }
            }
        } else {
            echo "<script>alert('Only JPG, JPEG, PNG, GIF, WEBP files are allowed.'); window.location='fprofile.php';</script>";
        }
    }
}
...
if(isset($_POST['upload_banner_pic'])) {
    $target_dir = "assets/uploads/FBanner/";
    $full_target_dir = "../" . $target_dir;
    ...
    if(isset($_FILES["banner_pic"]) && $_FILES["banner_pic"]["error"] == 0) {
        ...
        if(in_array($imageFileType, $allowed_types)) {
            if(move_uploaded_file($_FILES["banner_pic"]["tmp_name"], $target_file)) {
                $db_file_path = $target_dir . $file_name;
                $update_pic = "UPDATE farmerlogin SET banner_pic='$db_file_path' WHERE farmer_id='$para1'";
                if(mysqli_query($conn, $update_pic)) {
                    echo "<script>alert('Banner picture updated successfully!'); window.location='fprofile.php';</script>";
                    exit();
                }
            }
        } else {
...
<head>
    ...
    <title>KisanMitra - Farmer Profile</title>
...
<div class="image-banner flex items-center justify-center relative">
    ...
    <div class="relative z-10 text-center px-4 animate-fadeInUp">
        <div class="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-3 py-1 md:px-4 md:py-1.5 mb-3 md:mb-4">
            <i data-lucide="tractor" class="w-3 h-3 md:w-4 md:h-4 text-white"></i>
            <span class="text-white text-xs md:text-sm font-medium">Farmer Dashboard</span>
        </div>
        ...
        <p class="text-white/90 text-xs md:text-base mt-2 md:mt-3 flex flex-wrap items-center justify-center gap-1 md:gap-2">
            ...
            <?php echo htmlspecialchars($para11) ?>, <?php echo htmlspecialchars($para10) ?> - <?php echo htmlspecialchars($para9) ?>
        </p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="lg:col-span-1 animate-slideInLeft">
                <div class="main-card p-4 md:p-6 text-center">
                    <div class="profile-img-container mb-3 md:mb-4">
                        <img src="<?php echo '../' . $profile_pic; ?>" alt="Farmer Avatar" 
                    ...
                    <div class="grid grid-cols-2 gap-2 md:gap-3 mb-4 md:mb-5 pt-1 md:pt-2">
                        <div class="bg-terracotta/5 rounded-xl p-1.5 md:p-2">
                            <p class="text-xs text-text-light">Farmer Since</p>
                ...
                <div class="main-card p-4 md:p-5 lg:p-6">
                    <div class="flex flex-wrap justify-between items-center mb-4 md:mb-5">
                        <div>
                            ...
                            <p class="text-xs md:text-sm text-text-light mt-0.5 md:mt-1">Your account details and farming profile</p>
                        </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-x-4 md:gap-x-6 gap-y-0">
                        <?php
                        $profileDetails = [
                            'Farmer ID' => ['icon' => 'fingerprint', 'value' => $para1],
                            'Full Name' => ['icon' => 'user', 'value' => $para2],
                            'Email Address' => ['icon' => 'mail', 'value' => $para5],
                            'Mobile Number' => ['icon' => 'phone', 'value' => $para6],
                            'Gender' => ['icon' => 'users', 'value' => $para7],
                            'Date of Birth' => ['icon' => 'calendar', 'value' => $para8],
                            'State' => ['icon' => 'map', 'value' => $para9],
                            'District' => ['icon' => 'navigation', 'value' => $para10],
                            'City/Village' => ['icon' => 'home', 'value' => $para11, 'span' => true],
                        ];
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3 lg:gap-4 stats-grid animate-fadeInUp delay-200 mb-6 md:mb-8">
            <div class="stat-card">
                ...
                <p class="text-xs text-text-light">Products Sold</p>
            </div>
            <div class="stat-card">
                <div class="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 rounded-full bg-terracotta/10 flex items-center justify-center mx-auto mb-1 md:mb-2">
                ...
                <p class="text-lg md:text-xl lg:text-2xl font-bold text-primary-dark">0</p>
                <p class="text-xs text-text-light">Active Listings</p>
            </div>
            <div class="stat-card">
                <div class="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 rounded-full bg-terracotta/10 flex items-center justify-center mx-auto mb-1 md:mb-2">
                ...
                <p class="text-lg md:text-xl lg:text-2xl font-bold text-primary-dark">0</p>
                <p class="text-xs text-text-light">Orders Received</p>
            </div>
            <div class="stat-card">
                <div class="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 rounded-full bg-terracotta/10 flex items-center justify-center mx-auto mb-1 md:mb-2">
                    <i data-lucide="star" class="w-4 h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 text-terracotta"></i>
                </div>
                <p class="text-lg md:text-xl lg:text-2xl font-bold text-primary-dark">4.8</p>
            ...
            <div class="main-card p-4 md:p-5">
                <div class="flex flex-wrap items-center justify-between mb-3 md:mb-4">
                    <div class="flex items-center gap-2 md:gap-3">
                        ...
                        <div>
                            <h3 class="text-sm md:text-base lg:text-lg font-bold text-text-dark">Farm Location</h3>
                            <p class="text-xs text-text-light">Your registered farming area</p>
                        </div>
                    </div>
                ...
                <div class="pl-8 md:pl-12">
                    <p class="text-xs md:text-sm text-text-dark leading-relaxed break-words"><?php echo htmlspecialchars($para11); ?></p>
                    <p class="text-xs md:text-sm text-text-dark mt-1 break-words"><?php echo htmlspecialchars($para10); ?>, <?php echo htmlspecialchars($para9); ?></p>
            ...
            <div class="main-card p-4 md:p-5">
                <div class="flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
                    <div class="w-8 h-8 md:w-10 md:h-10 rounded-full bg-secondary-green/10 flex items-center justify-center">
                        <i data-lucide="tractor" class="w-4 h-4 md:w-5 md:h-5 text-secondary-green"></i>
                    </div>
                    <div>
                        <h3 class="text-sm md:text-base lg:text-lg font-bold text-text-dark">Farming Details</h3>
                        <p class="text-xs text-text-light">Your agricultural profile</p>
                    </div>
                </div>
                <div class="space-y-2 md:space-y-3 pl-8 md:pl-12">
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="leaf" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">Primary Crop</span>
                        </div>
                        <span class="text-xs <?php echo $primary_crop != 'Not Specified' ? 'text-secondary-green' : 'text-text-light'; ?> font-medium break-words"><?php echo htmlspecialchars($primary_crop); ?></span>
                    </div>
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="maximize" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">Farm Size</span>
                        </div>
                        <span class="text-xs <?php echo $farm_size != 'Not Specified' ? 'text-secondary-green' : 'text-text-light'; ?> font-medium"><?php echo htmlspecialchars($farm_size); ?> Acres</span>
                    </div>
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="calendar" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">Experience</span>
                        </div>
                        <span class="text-xs text-text-dark"><?php echo htmlspecialchars($experience_years); ?> Years</span>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Bank Details & KYC Cards -->
        <div class="grid md:grid-cols-2 gap-4 md:gap-6 mt-5 md:mt-6 animate-fadeInUp delay-300">
            <div class="main-card p-4 md:p-5">
                <div class="flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
                    <div class="w-8 h-8 md:w-10 md:h-10 rounded-full bg-terracotta/10 flex items-center justify-center">
                        <i data-lucide="banknote" class="w-4 h-4 md:w-5 md:h-5 text-terracotta"></i>
                    </div>
                    <div>
                        <h3 class="text-sm md:text-base lg:text-lg font-bold text-text-dark">Bank Details</h3>
                        <p class="text-xs text-text-light">Payment information</p>
                    </div>
                </div>
                <div class="space-y-2 md:space-y-3 pl-8 md:pl-12">
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="user" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">Account Holder</span>
                        </div>
                        <span class="text-xs text-text-dark break-words"><?php echo htmlspecialchars($para2); ?></span>
                    </div>
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="credit-card" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">Account Number</span>
                        </div>
                        <span class="text-xs text-text-dark break-words"><?php echo htmlspecialchars($bank_account); ?></span>
                    </div>
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="building-2" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">IFSC Code</span>
                        </div>
                        <span class="text-xs text-text-dark"><?php echo htmlspecialchars($ifsc_code); ?></span>
                    </div>
                </div>
            </div>
            ...
            <div class="main-card p-4 md:p-5">
                <div class="flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
                    <div class="w-8 h-8 md:w-10 md:h-10 rounded-full bg-secondary-green/10 flex items-center justify-center">
                        <i data-lucide="award" class="w-4 h-4 md:w-5 md:h-5 text-secondary-green"></i>
                    </div>
                    <div>
                        <h3 class="text-sm md:text-base lg:text-lg font-bold text-text-dark">KYC & Certifications</h3>
                        <p class="text-xs text-text-light">Government schemes & certificates</p>
                    </div>
                </div>
                <div class="space-y-2 md:space-y-3 pl-8 md:pl-12">
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="id-card" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">Aadhar Number</span>
                        </div>
                        <span class="text-xs text-text-dark break-words"><?php echo htmlspecialchars($aadhar_number); ?></span>
                    </div>
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="file-text" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">PAN Number</span>
                        </div>
                        <span class="text-xs text-text-dark break-words"><?php echo htmlspecialchars($pan_number); ?></span>
                    </div>
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="leaf" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">Organic Certified</span>
                        </div>
                        <span class="text-xs <?php echo $organic_certified == 'Yes' ? 'text-secondary-green' : 'text-text-light'; ?>"><?php echo $organic_certified; ?></span>
                    </div>
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="users" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">FPO Member</span>
                        </div>
                        <span class="text-xs <?php echo $fpo_member == 'Yes' ? 'text-secondary-green' : 'text-text-light'; ?>"><?php echo $fpo_member; ?></span>
                    </div>
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-1 md:gap-2">
                            <i data-lucide="coins" class="w-3 h-3 md:w-4 md:h-4 text-text-light"></i>
                            <span class="text-xs md:text-sm text-text-dark">PM-KISAN Status</span>
                        </div>
                        <span class="text-xs text-secondary-green"><?php echo $pm_kisan_status; ?></span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="flex justify-center items-center min-h-screen p-3 md:p-4">
        <div class="modal-content">
            <div class="modal-header">
                <div>
                    <h4 class="text-lg md:text-xl lg:text-2xl font-bold">🌾 Edit Farmer Profile</h4>
                    <p class="text-xs md:text-sm opacity-80">Update your farming profile and personal information</p>
                </div>
                ...
                <form method="POST" autocomplete="off">
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select name="gender" required class="form-input text-sm">
                                <option value="">Select Gender</option>
                                <option value="Male" <?php echo $para7 == 'Male' ? 'selected' : ''; ?>>Male</option>
                                <option value="Female" <?php echo $para7 == 'Female' ? 'selected' : ''; ?>>Female</option>
                                <option value="Other" <?php echo $para7 == 'Other' ? 'selected' : ''; ?>>Other</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="dob" value="<?php echo $para8 ?>" required class="form-input text-sm">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select onChange="getdistrict(this.value);" name="state" id="state" required class="form-input text-sm">
                                ...
                                while($state_row = mysqli_fetch_array($state_query)) { 
                                    $selected = ($state_row['StateName'] == $para9) ? 'selected' : '';
                                ?>
                        ...
                        <div class="form-group">
                            ...
                            <select name="district" id="district-list" required class="form-input text-sm">
                                <option value="<?php echo htmlspecialchars($para10); ?>"><?php echo htmlspecialchars($para10); ?></option>
                            </select>
                        </div>

                        <div class="form-group md:col-span-2">
                            <label class="form-label text-xs md:text-sm">
                                <i class="fas fa-home"></i> City/Village *
                            </label>
                            <input type="text" name="city" value="<?php echo htmlspecialchars($para11) ?>" required class="form-input text-sm">
                        </div>

                        <div class="form-group">
                            <label class="form-label text-xs md:text-sm">
                                <i class="fas fa-maximize"></i> Farm Size (Acres)
                            </label>
                            <input type="text" name="farm_size" value="<?php echo htmlspecialchars($farm_size); ?>" class="form-input text-sm" placeholder="Enter farm size">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="primary_crop" value="<?php echo htmlspecialchars($primary_crop); ?>" class="form-input text-sm" placeholder="Enter primary crop">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="experience_years" value="<?php echo htmlspecialchars($experience_years); ?>" class="form-input text-sm" placeholder="Years of experience">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="aadhar_number" value="<?php echo htmlspecialchars($aadhar_number); ?>" class="form-input text-sm" placeholder="12-digit Aadhar">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="pan_number" value="<?php echo htmlspecialchars($pan_number); ?>" class="form-input text-sm" placeholder="PAN number">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="bank_account" value="<?php echo htmlspecialchars($bank_account); ?>" class="form-input text-sm" placeholder="Account number">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="ifsc_code" value="<?php echo htmlspecialchars($ifsc_code); ?>" class="form-input text-sm" placeholder="IFSC code">
                        </div>
                        ...
                        <div class="form-group md:col-span-2">
                            <label class="form-label text-xs md:text-sm">
                                <i class="fas fa-leaf"></i> Certifications
                            </label>
                            <div class="checkbox-group">
                                <label class="checkbox-item">
                                    <input type="checkbox" name="organic_certified" value="1" <?php echo $organic_certified == 'Yes' ? 'checked' : ''; ?>>
                                    <span class="text-xs md:text-sm">Organic Certified</span>
                                </label>
                                <label class="checkbox-item">
                                    <input type="checkbox" name="fpo_member" value="1" <?php echo $fpo_member == 'Yes' ? 'checked' : ''; ?>>
                                    <span class="text-xs md:text-sm">FPO Member</span>
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col-reverse sm:flex-row justify-center gap-2 md:gap-3 mt-6 md:mt-8 pt-4 border-t border-border">
                        ...
                        <button name="farmerupdate" class="btn-terracotta text-sm md:text-base w-full sm:w-auto justify-center">
...
function getdistrict(val) {
    $.ajax({
        ...
        success: function(data){
            ...
            // After loading districts, try to select the current district if it exists
            var currentDistrict = "<?php echo htmlspecialchars($para10); ?>";
            if(currentDistrict && currentDistrict !== '') {
                $("#district-list option").each(function() {
                    if($(this).val() == currentDistrict || $(this).text() == currentDistrict) {
